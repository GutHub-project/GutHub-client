import { publicApiInstance } from '@repo/shared';
...
export const dietApi = {
  ...
  searchFoods: async (keyword: string): Promise<Food[]> => {
    const response = await publicApiInstance.get<SearchFoodResponse>(
      '/diets/search-foods',
      {
        params: { keyword },
      }
  ...
  createDiet: async (data: CreateDietRequest): Promise<DietLog[]> => {
    const response = await publicApiInstance.post<CreateDietResponse>(
      '/diets',
      data
    );
    return response.data.data;
  ...
  getDiet: async (dietLogId: number): Promise<DietLog> => {
    const response = await publicApiInstance.get<{ code: string; data: DietLog; message: string }>(
      `/diets/${dietLogId}`
    );
    return response.data.data;
  },
  ...
  updateDiet: async (dietLogId: number, data: Partial<DietItem>): Promise<DietLog> => {
    const response = await publicApiInstance.put<{ code: string; data: DietLog; message: string }>(
      `/diets/${dietLogId}`,
      data
    );
    return response.data.data;
  ...
  deleteDiet: async (dietLogId: number): Promise<void> => {
    await publicApiInstance.delete(`/diets/${dietLogId}`);
  },
};
